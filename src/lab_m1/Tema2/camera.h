...
namespace world_camera
{
    class Camera
    {
     public:
        ...
        void SetThirdPerson(const glm::vec3 &playerPos)
        {
            position = glm::vec3(playerPos.x, playerPos.y, playerPos.z - distanceToTarget);
            forward = glm::normalize(playerPos - position);
            up = glm::vec3(0, 1, 0);
            right = glm::cross(forward, up);
            up = glm::cross(right, forward);
            RotateThirdPerson_OX(glm::radians(-45.f));
        }

        void SetFirstPerson(const glm::vec3& playerPos) 
        {
            position = glm::vec3(playerPos.x, playerPos.y, playerPos.z + distanceFromHead);
            forward = glm::normalize(position - playerPos);
            up = glm::vec3(0, 1, 0);
            right = glm::cross(forward, up);
            up = glm::cross(right, forward);
        }
